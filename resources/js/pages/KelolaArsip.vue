...
const allDocuments = ref(
  (Array.isArray(page.props.arsip) ? page.props.arsip : []).map((item: any) => ({
    id: item.id,
    title: item.judul,
    nomor: item.nomor,
...
const filteredDocuments = computed(() => {
  return allDocuments.value.filter((item: { jenis: string }) => {
    return filterJenis.value ? item.jenis === filterJenis.value : true
  })
})
